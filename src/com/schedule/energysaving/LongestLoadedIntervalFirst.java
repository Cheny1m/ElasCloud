...
// */
//
//public class LongestLoadedIntervalFirst extends OfflineAlgorithm {
...
//
//    public LongestLoadedIntervalFirst(){
...
//
//        int SlotOne[] = new int[10000];
//        int SlotTwo[] = new int[10000];
//        int SlotThree[] = new int[10000];
//
//        //计算每个队列，每个时隙所需要的最少机器数
//        for(int i = FindMinStartTime(vmQueue); i < FindMaxEndTime(vmQueue) ; i++){
//            for(int j = 0 ; j < vmQueue.size() ; i++){
//                if (vmQueue.get(j).getVmType() > 0 && vm.getVmType() < 4){
//                    if(vmQueue.get(j).getStartTime()<= i && vmQueue.get(j).getEndTime() > i){
//                        SlotOne[i] += vmQueue.get(j).getCpuTotal();
//                    }
//                }
//                else if(vmQueue.get(j).getVmType() >= 4 && vm.getVmType() < 7){
//                    if(vmQueue.get(j).getStartTime()<= i && vmQueue.get(j).getEndTime() > i){
//                        SlotTwo[i] += vmQueue.get(j).getCpuTotal();
//                    }
//                }
//                else{
//                    if(vmQueue.get(j).getStartTime()<= i && vmQueue.get(j).getEndTime() > i){
//                        SlotThree[i] += vmQueue.get(j).getCpuTotal();
//                    }
//                }
//            }
//            SlotOne[i] = (int)Math.ceil(SlotOne[i]/arr_dc.get(dataCenterIndex).getArr_lbf().get(rackIndex).getPmQueueOne().get(0).getCpuTotal());
//            SlotTwo[i] = (int)Math.ceil(SlotTwo[i]/arr_dc.get(dataCenterIndex).getArr_lbf().get(rackIndex).getPmQueueTwo().get(0).getCpuTotal());
//            SlotThree[i] = (int)Math.ceil(SlotThree[i]/arr_dc.get(dataCenterIndex).getArr_lbf().get(rackIndex).getPmQueueThree().get(0).getCpuTotal());
//        }
//
//        int flag = SlotOne[0];
//        int count = 0;
