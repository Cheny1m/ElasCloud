...
*/
public class LoadEnergy extends OfflineAlgorithm {
...
//    int SumPMThree = 1;
    ...
    public void allocate(ArrayList<VirtualMachine> p_vmQueue, ArrayList<DataCenter> p_arr_dc) {
        ...
        double threshold = TotalLoad * 1.0 / (NumbOfPM * (LoadBalanceFactory.MAXTIME) * p_arr_dc.get(0).getArr_lbf().get(0).getPmQueueOne().get(0).getCpuTotal());
...
//        if(f < 0.5) f = 0.5;
        ...
        CMBound =(int) Math.ceil(f * (LoadBalanceFactory.MAXTIME) * p_arr_dc.get(0).getArr_lbf().get(0).getPmQueueOne().get(0).getCpuTotal());
        ...
        if (checkResourceAvailble(vm2, pm2)) {
            ...
            //3.方案3，超过当前负载的界限，实现迁移
                ...
                if(pm2.getTurnStatus()){
                    DataCenterFactory.print.println("超过此物理机整个负载的界限，实现迁移，同时关闭物理机 PM " + pm2.getNo());
...
//    }
    ...
    private int MigrateAllocate(VirtualMachine vm3, PhysicalMachine pm3){
        ...
        int starttime = vm3.getStartTime();
        //记录第一秒的资源是否够用
        change = (pm3.resource.get(starttime).getCpuUtility()  - vm3.getCpuTotal() >= 0);
        //change = (pm3.resource.get(starttime).getCpuUtility()  - vm3.getCpuTotal() >= pm3.getCpuTotal() * (1-f) );
        for (int t = vm3.getStartTime(); t <= vm3.getEndTime(); t++) {
            //查找当前slot区间的状态
            flag = (pm3.resource.get(t).getCpuUtility()  - vm3.getCpuTotal() >= 0);
            //flag = (pm3.resource.get(t).getCpuUtility()- vm3.getCpuTotal() >= pm3.getCpuTotal() * (1-f));
            //如果两者状态相同且不是最后的一秒；那么继续累加区间
            if(flag == change && t != vm.getEndTime()){
                //只要不是把区间分割完成
                continue;
                ...
                //Collections.sort(arr_dc.get(dataCenterIndex).getArr_lbf().get(rackIndex).getPmQueueOne(), new SortByDEPMUtility());
                //更新当前活跃物理机数量
                EffeNumb = NumberofSpanEffePM(arr_dc.get(dataCenterIndex).getArr_lbf().get(rackIndex).getPmQueueOne(),starttime);
                if(EffeNumb == 0){
                    index = 0;
                    EffePMID.add(arr_dc.get(dataCenterIndex).getArr_lbf().get(rackIndex).getPmQueueOne().get(0).getNo());
                    EffePMIndex.add(0);
                ...
                //DataCenterFactory.print.println("当前活跃物理机数量：" + EffeNumb);
                if(change){
                    DataCenterFactory.print.println("直接分配部分区间");
                    //分配给当前利用率最小的
                    //PhysicalMachine pm4 = arr_dc.get(dataCenterIndex).getArr_lbf().get(rackIndex).getPmQueueOne().get(EffeNumb-1);
                    //PhysicalMachine pm4 = arr_dc.get(dataCenterIndex).getArr_lbf().get(rackIndex).getPmQueueOne().get(EffePMIndex.get(EffeNumb-1));

                    //分配给当前利用率最小的机器
                    //pm3 = arr_dc.get(dataCenterIndex).getArr_lbf().get(rackIndex).getPmQueueOne().get(EffePMIndex.get(EffeNumb-1));

                    //如果上个区间是可分配区间，那么就直接分配，同时更新资源
                    //不过不重新修改pm3，那么就是最佳分配的pm
                    AdvanceUpdateResource(pm3,vm4);
                    //allocateVm(allocatedDataCenterID, allocatedRackID, vm4, pm3);
                }
                else{
                    ...
                    for(int i = 0;i < pmTotalNum ; i++){
                        if(EffePMID.contains(arr_dc.get(dataCenterIndex).getArr_lbf().get(rackIndex).getPmQueueOne().get(i).getNo())){
                            continue;
                        }
                        else{
                            //查找到第一个未开机的物理机
                            index = i;
                            //DataCenterFactory.print.println("当前第一个未开机的物理机NO为：" + i);
                            break;
                        }
                    }
                    triedAllocationTimes++;
    ...
    private void MigrateAllocatebyCMBound(VirtualMachine vm3, PhysicalMachine pm3){
        ...
        if(RemainingDration == 0 ){
            ...
            //剩余的新区间
            VirtualMachine vm4 = new VirtualMachine(vmQueue.size() + NumberofAllocate,vm3.getStartTime() + RemainingDration,vm3.getEndTime(),vm3.getVmType());
            //分配当前区间
    ...
    //查找能接受的最长区间迁移
    private int longestIntervalAllocate(VirtualMachine v_vm,ArrayList<PhysicalMachine> P_PmQueue){
        int starttime = v_vm.getStartTime();
        int endtime = v_vm.getEndTime();
        int count = 0;
        int minInterval = Integer.MAX_VALUE;
        //返回PM索引
        int resindex = 0;
        for(int i = 0 ; i < EffeNumb ; i++){
            for(int j = starttime ; j < endtime ; j++){
                //查找资源不够用的区间
                if(P_PmQueue.get(EffePMIndex.get(i)).resource.get(j).getCpuUtility() * 0.88  - v_vm.getCpuTotal() < 0){
                    //if(P_PmQueue.get(EffePMIndex.get(i)).resource.get(j).getCpuUtility() - v_vm.getCpuTotal() < P_PmQueue.get(EffePMIndex.get(i)).getCpuTotal() * (1-f)){
                    count ++;
                }
                else break;
            }
